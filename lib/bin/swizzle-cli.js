...
var rc = _fileSystem.sfs.loadRcConfig({ rcFiles: _fileSystem.sfs.getRcFilePathsIfExists() });
var conf = new _config.SwizzleConfig(_fileSystem.sfs.loadSwizzleConfig({ file: './swizzle.json', rc: rc }));
var swizzle = new _swizzle.Swizzle(conf, _fileSystem.sfs);

_commander2.default.version(_package2.default.version);

_commander2.default.command('add-param').alias('ap').description('add a parameter to swizzle.json').option('-g, --generated', 'generated parameter so do not prompt user').option('-n, --name <name>', 'name of parameter to add').option('-d, --desc <desc>', 'description of parameter').option('-v, --default-value <defaultValue>', 'default value of parameter').action(function (options) {

	console.log('add-param %s--name \'%s\' --desc %s --default-value %s', options.generated ? '-g ' : '', options.name, options.desc, options.defaultValue);

...
var SwizzleFileSystem = exports.SwizzleFileSystem = function () {
	...
	_createClass(SwizzleFileSystem, [{
		...
		value: function saveSwizzleConfig(_ref2) {
			...
			if (conf.stacks) {
				Object.keys(conf.stacks).forEach(function (stackName) {
					...
					var stackParams = conf.params.reduce(function (result, param) {
						if (param.noSave) {
					...
					var resolvedFile = _path2.default.resolve(_this2.cwd(), file);
					var resolvedStackFile = _path2.default.resolve(_this2.cwd(), stack.file);
					if (stack.file && resolvedStackFile === resolvedFile) {
						(0, _lodash2.default)(swizzleJson.stacks, entry);
					} else {
						if (_this2.isFile(resolvedStackFile)) {
							json = _this2.readJsonFile({ file: resolvedStackFile });
						}
						if (!json.stacks) {
							json.stacks = {};
						}
						Object.assign(json.stacks, entry);
						_this2.writeJsonFile({ file: resolvedStackFile, json: json });
		...
		value: function getSwizzleJsonFilePath() {
			return _path2.default.resolve(this.cwd(), swizzleFileName);
		}
	}, {
		key: 'readJsonFile',
